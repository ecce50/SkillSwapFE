...
function ClassSessions({ sessions, classId, setSessions }) {
  ...
  // Updated to track the specific session being edited
  ...
  // Modified handleEdit function to track the specific session being edited
  const handleEdit = (sessionId) => {
    setEditedSessionId(sessionId); // Set the specific session ID being edited
  ...
  // Cancel edit mode by clearing the editedSessionId
  const handleCancelEdit = () => {
    setEditedSessionId(null); // Reset edit mode for all sessions
    setUpdatedSession({});
  };
  ...
  const handleSaveEditSession = async (sessionId) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setEditedSessionId(null); // Close the edit mode after saving
    ...
    <div style={{ backgroundColor: "red" }}>
      ...
      {sessions.map((aSession) => (
        <div key={aSession._id} id={aSession._id}>
          {/* Only render the edit form for the session being edited */}
          {editedSessionId === aSession._id ? (
            <>
              <label>
                Date:
                <DatePicker
...
//     }));
//     console.log("These are prevEditedSessions: ", editedSessions);
//     toggleEditMode();
//     const sessionToEdit = sessions.find((c) => c._id === sessionId);
//     const dateTime = new Date(sessionToEdit.dateTime);
//     setUpdatedSession({
